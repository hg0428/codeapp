...
def EncodeRspFileList(args, quote_cmd):
    ...
    return f"{program} " + " ".join(
        QuoteForRspFile(arg, quote_cmd) for arg in args[1:]
    )
...
class MsvsSettings:
    ...
    def AdjustLibraries(self, libraries):
        ...
        return [
            ...
            and not lib.lower().endswith(".obj")
            else lib
            for lib in libs
        ]
    ...
    def GetLdflags(
        ...
        if stack_reserve_size := self._Setting(
        ...
        if all("NXCOMPAT" not in flag for flag in ldflags):
...
def _FormatAsEnvironmentBlock(envvar_dict):
    ...
    nul = "\0"
    return (
        "".join(f"{key}={value}{nul}" for key, value in envvar_dict.items())
        + nul
    )
...
def GenerateEnvironmentFiles(
    ...
    for arch in archs:
        ...
        f = open_out(os.path.join(toplevel_build_dir, f"environment.{arch}"), "w")
        f.write(env_block)
        f.close()
...
def VerifyMissingSources(sources, build_dir, generator_flags, gyp_to_ninja):
    ...
    if int(generator_flags.get("msvs_error_on_missing_sources", 0)):
        ...
        if missing := [x for x in relative if not os.path.exists(x)]:
