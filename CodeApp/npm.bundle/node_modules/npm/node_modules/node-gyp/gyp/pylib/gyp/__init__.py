...
def DebugOutput(mode, message, *args):
    if "all" in gyp.debug or mode in gyp.debug:
        ...
        try:
            if f := traceback.extract_stack(limit=2):
                ctx = f[0][:3]
        except Exception:
            pass
        if args:
...
def gyp_main(args):
    ...
    if not options.generator_output and options.use_environment:
        if g_o := os.environ.get("GYP_GENERATOR_OUTPUT"):
    ...
    if home_dot_gyp:
        ...
        if os.path.exists(default_include):
            print(f"Using overrides found in {default_include}")
        ...
        if options.configs:
            ...
            for conf in options.configs:
                if conf not in valid_configs:
                    raise GypError(f"Invalid config specified via --build: {conf}")
